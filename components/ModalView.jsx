...
export default function ModalView({ data }) {
 ...
 const getVideo = async (id) => {
  try {
   ...
   setPlay(
    response.data.results.link_download
     .filter((link) => link.item === "Fembed")[0]
     .link.replace("https://layarkacaxxi.icu/f/", "")
   );
           ...
           <div className="absolute bottom-5 md:bottom-10 flex w-full items-center justify-between px-5 md:px-10">
            <div>
             <a
              ...
              href={meta.server_embed.P2P ? 
        meta.server_embed.P2P.link 
        : meta.server_embed.CAST ?
            meta.server_embed.CAST.link 
            : '#'}
            target='blank'
